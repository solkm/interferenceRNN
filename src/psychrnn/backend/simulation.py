...
class BasicSimulator_linOut(Simulator):
    """
    See :class:`Simulator` for arguments.
    """
    ...
    def rnn_step(self, state, rnn_in, t_connectivity):
        ...
        new_output = np.matmul(new_state, np.transpose(self.W_out)) + self.b_out
...
class BasicSimulator_dropout(Simulator):
    """:class:`Simulator` implementation for :class:`psychrnn.backend.models.basic.Basic` and for :class:`psychrnn.backend.models.basic.BasicScan`.
    ...
    def rnn_step(self, state, rnn_in, t_connectivity):
        ...
        dropped_units = np.random.randint(0, new_state.shape[1], size=(new_state.shape[0], int(new_state.shape[1]*0.3)))
        for i in range(new_state.shape[0]):
            new_state[i, dropped_units[i]] = 0
