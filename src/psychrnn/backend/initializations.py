...
class WeightInitializer(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.load_weights_path is not None:
            ...
            self.initializations['transfer_function'] = kwargs.get(
                'transfer_function', tf.nn.relu
            )
            self.initializations['W_in'] = kwargs.get(
                'W_in', self.rand_init(self.initializations['input_connectivity'])
            )
            assert(self.initializations['W_in'].shape == (N_rec, N_in))

            self.initializations['W_out'] = kwargs.get(
                'W_out', self.rand_init(self.initializations['output_connectivity'])
            )
            assert(self.initializations['W_out'].shape == (N_out, N_rec))

            self.initializations['W_rec'] = kwargs.get(
                'W_rec', self.rand_W_rec_init(self.initializations['rec_connectivity'])
            )
    ...
    def get_rand_init_func(self, which_rand_init):
        ...
        Arguments:
            which_rand_init (str): Maps to [which_rand_init]_init. 
            Options are 'const_unif', 'const_gauss', 'glorot_unif', 'glorot_gauss'.
    ...
    def get(self, tensor_name):
        ...
        return tf.constant_initializer(self.initializations[tensor_name].astype(np.float32))
